...
"""
Python bindings for SCTP. It is compatible with kernel-level SCTP implementations 
with BSD/Sockets API.
...
SCTP sockets do NOT inherit from socket._socketobject, instead they CONTAIN a 
standard Python socket, and DELEGATE unknown calls to it. So it should fit in most 
places where a "real" socket is expected.

When receiving data, it is also possible to receive metadata in the form of events. 
An event will be one of the following classes:
- sndrcvinfo()
- notification()
- assoc_change()
- paddr_change()
...
Every SCTP socket has a number of properties. Two "complex" properties, that contain 
a number of sub-properties, are:
- sctpsocket.events = event_subscribe()
- sctpsocket.initparams = initparams()

Please take a look in the classes' documentation to learn more about respective 
sub-properties.

Most SCTP-specific macros are available as constants in Python, so an experienced 
SCTP/C API programmer will feel at home at pysctp. The only difference is that constants 
used only inside a particular event, are defined as class constants and not as module 
constants, to avoid excessive namespace pollution.
"""
...
class initmsg(object):
    """
    Object used in explicit opening of SCTP associations in UDP-style sockets 
    without passing any user-data message (UNIMPLEMENTED in pysctp, probably needs 
    direct sendmsg() use)

    User will be better off it instantiate this class via initparams.initmsg() 
    constructor, because it fills initmsg with socket's defaults.
    """
    def __init__(self):
        self.num_ostreams = 0
...
class initparams(object):
    """
    SCTP default initialization parameters. New associations are opened with the 
    properties specified in this object.
    ...
    Setting of properties will automatically call flush() (that will setsockopt the 
    socket) unless "autoflush" is set to False.

    User should never need to instantiate this class directly. For every property, 
    there is a pair of get_P and set_P methods, that user can, but should not need, 
    to call directly.
    """

    def __init__(self, container):
        """
        The container object passed as parameter will (I hope) be a sctpsocket() 
        descendant. It should offer the _get_initparams() and _set_initparams() for 
        the socket it represents.
        """
        self.autoflush = True
    ...
    def flush(self):
        """
        Flushes the initialization properties to the socket (setsockopt). If autoflush
        equals True (the default), any change to properties will call this automatically.
        """
        self.container._set_initparams(self.__dict__)

    def initmsg(self):
        """
        Builds a initmsg() object containing the same properties as the socket 
        initalization parameters. The initmsg() can then be changed and used to open 
...
class sndrcvinfo(object):
    """
    Send/receive ancilliary data. In PySCTP, this object is only used to *receive* 
    ancilliary information about a message. The user should never need to instantiate 
    this class.
...
class notification(object):
    """
    Base class for all notification objects. Objects of this particular type should 
    never appear in message receiving. If it does, it means that PySCTP received a 
    notification it does not understand (probably a "new" event type), and author 
    must be warned about this.

    The "type" and "flags" fields belong to this class because they appear in every 
    notification. This class contains "type_*" constants that identify every notification 
    type. On the other hand, "flags" meanings are particular for every notification type.
...
class assoc_change(notification):
    ...
    It seems to lack remote address/port pair, but remember that recvmsg() and sctp_recv() 
    return the address for every (either data or event) message, so the application 
    can have full knowledge about the new connection.
...
class remote_error(notification):
    ...
    protocol does not use this feature, such messages don't need to be handled.

    It will NOT be received when an "actual", or low-level, error occurs. For such 
    errors, assoc_change() objects are passed instead.
...
class shutdown_event(notification):
    """
    Shutdown event. This event is received when an association goes down. The only 
    relevant attribute is assoc_id.
    
    If user has subscribed to receive assoc_change notifications, it will receive 
    at least 2 messages (assoc_change and shutdown_event) for an association shutdown.
    In addition, if the association was down by some problem, more error notifications 
    will be received before this event.
...
class adaptation_event(notification):
    """
    Adaption indication event. It signals that the remote peer requests an specific 
    adaptation layer.

    In SCTP, you can pass an optional 32-bit metadata identifier along each message, 
    called "ppid". For protocols that actively use this feature, it is like a subprotocol 
    identifier, or "adaptation layer". The message interpreter is chosen by this value. 
    This is mostly for telephony signaling-related protocols.

    When receiving this event, the new messages should be sent with the new requested ppid.

    For protocols that use multiple ppids, or do not take advantage of ppid at all, 
    this notification is useless and can be ignored, or even better, unsubscribed.

    The user should never need to instantiate this directly. This notification will 
    be received only if user subscribed to receive that (see event_subscribe class for 
    details).
    """
    def __init__(self, values=None):
...
class pdapi_event(notification):
    """
    Partial delivery event. This event is received when a partial delivery event occurs.
...
def notification_factory(raw_notification):
    """
    This function builds a notification based on a raw dictionary received from 
    the bottom-half (C language) of pysctp. The user should never need to call this 
    directly.

    If the raw notification is of an unknown type, a "notification" superclass object 
    is returned instead, so the user has a chance to test "type" and deal with that. 
    A warning is sent to the stderr. It can well happen if SCTP is extended and the 
    user is using an old version of PySCTP...

    If sctpsocket.unexpected_event_raises_exception (default=False) is set to True, 
    an unknown event will raise an exception (it is not raised by this function, but 
    it seemed important to note it here too).
    """
    try:
...
class paddrinfo(object):
    """
    Peer Address information object class. The user should never need to instantiate 
    this directly.  It is received via get_paddrinfo() method call. It is read-only, 
    you cannot set anything by passing this object.
...
def features():
    """
    Returns a bitmap of features possessed by the SCTP underlying implementation. 
    Individual bits can be tested against HAVE_* constants.

    These flags may not be completely trustable, even in the sense that some feature 
    may be available despite absence of flag. LK-SCTP 1.0.3 has no HAVE_SCTP_PRSCTP macro, 
    but Linux implements PR-SCTP extension since kernel 2.6.10...
    """
...
class sctpsocket(object):
    ...
    autoclose: For UDP-style sockets, sets/gets the auto-closing timeout for idle 
          associations, in seconds. A value of 0 means that no automatic close will be 
          done. This property does not work for TCP-style sockets.
    ...
    def bindx(self, sockaddrs, action=BINDX_ADD):
        ...
        Raises an exception if bindx() is not successful.
        """
    ...
    def connectx(self, sockaddrs, assoc_id=None):
        ...
        Warning: not all SCTP implementations support connectx(). It will raise 
        a RuntimeError() if not supported.
        """
    ...
    def sctp_recv(self, maxlen):
        ...
        maxlen: The maximum message size that can be received. If bigger messages are
                received, they will be received in fragments (non-atomically). The 
                application must choose this carefully if it wants to keep the atomicity 
                of messages!
    ...
    def peeloff(self, assoc_id): 
        ...
        if fd < 0:
            raise IOError("assoc_id does not correspond to any open association")
    ...
    def accept(self): 
        """
        Frontend for socket.accept() method. Does exactly the same thing, except 
        that returns an sctpsocket_tcp object instead of a standard Python socket. 
        May throw the same exceptions as socket.accept()
    ...
    def get_status(self, assoc_id = 0):
        ...
        assoc_id: The association ID of the association. Must be zero or not passed 
                  at all for TCP-style sockets.
    ...
    def get_paddrinfo(self, assoc_id, sockaddr):
        ...
        sockaddr: Address/port tuple. Due to the nature of the information, it can not
                  be a wildcard (there is no "association-wide" information here); 
                  a concrete peer address must be passed.

        Returns: paddrinfo instance
    ...
    def get_assocparams(self, assoc_id = 0):
        ...
        assoc_id: The association ID of the association. Must be zero or not passed at 
                  all for TCP-style sockets. If you pass a zero for UDP-style sockets, 
                  it will return the default parameters of the whole endpoint.
        
        Returns: assocparams instance
        """
        s = assocparams()
        s.assoc_id = assoc_id
    ...
    def set_assocparams(self, obj):
        """
        Sets parameters for a SCTP association. Parameters:

        obj: assocparams() object containing the assoc_id of the association be
        ...
        """
        _sctp.set_assocparams(self._sk.fileno(), obj.__dict__)
    ...
    def get_paddrparams(self, assoc_id, sockaddr):
        ...
        Warning: It seems not to work totally in TCP-style sockets, in client side.
                 Only the wildcard address ("",0) seems to work. (we do not know the reason)

        Returns: paddrparams instance
    ...
    def set_paddrparams(self, obj):
        """
        Sets peer address parameters for a SCTP association. Parameters:

        obj: paddrparams() object containing the assoc_id/address of the peer to be
        ...
        """
        _sctp.set_paddrparams(self._sk.fileno(), obj.__dict__)
    ...
    def get_rtoinfo(self, assoc_id=0):
        ...
        assoc_id: The association ID of the association. Must be zero or not passed 
                  at all for TCP-style sockets. If zero is passed for UDP-style sockets, 
                  the information refers to the socket defaults, and not any particular
                  opened association.
        
        Returns: rtoinfo instance
        """
        s = rtoinfo()
        s.assoc_id = assoc_id
    ...
    def set_rtoinfo(self, obj):
        """
        Sets RTO parameters for a SCTP association. Parameters:

        obj: rtoinfo() object containing the assoc_id of the association to be
        ...
        """
        _sctp.set_rtoinfo(self._sk.fileno(), obj.__dict__)
    ...
    def set_streamid(self, newVal):
        ...
        if not isinstance(newVal, int) or newVal < 0 or newVal > 65535:
            raise ValueError('streamid shall be a valid unsigned 16 bits integer')
...
class sctpsocket_udp(sctpsocket):
    ...
    def __init__(self, family, sk=None):
        ...
        If "sk" is passed, it will use the passed socket instead. Make sure it 
        matches address family, protocol (SCTP) and style. In general, users 
        should never need to pass this parameter.
        """
