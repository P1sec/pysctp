...
/* Python 3 module initialization routine */
...
static struct PyModuleDef moduledef = {
        ...
        "_sctp",
        "SCTP libsctp API bindings for Linux",
...
#ifdef DEBUG
    printf("[DEBUG to_sockaddr] sockaddr result is family: 0x%x, s_addr: 0x%x, port: 0x%x, slen: %d\n", 
...
static PyObject* connectx(PyObject* dummy, PyObject* args)
{
    ...
    if (sctp_connectx(fd, saddrs, addrcount, &id)) {
        ...
        if(PyDict_Check(dict)) PyDict_SetItemString(dict, "assoc_id", PyLong_FromLong(id));
        ret = Py_None; Py_INCREF(ret);
    }
